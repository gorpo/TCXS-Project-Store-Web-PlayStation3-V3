...
if (!empty($_POST)) {
    ...
    if ($validacao) {
        ...
        $sql = "UPDATE playstation_psp  set titulo=:titulo, descricao=:descricao, content_id=:content_id, imagem=:imagem, cadastro=now() , link=:link  WHERE id=:id";
        $q = $pdo->prepare($sql);
    ...
    $pdo = Database::conectar();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT * FROM playstation_psp where id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($id));
...
<head>
    <meta charset="UTF-8">
      ...
      <title>TCXS Project | UPDATE PSP GAME</title>
</head>
<body>
<div class="container">
